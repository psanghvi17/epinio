...
func ensurePVC(ctx context.Context, cluster *kubernetes.Cluster, config StagingStorageValues, pvcName string, labels map[string]string) error {
...
func Stage(c *gin.Context) apierror.APIErrors {
	...
	if !params.HelmValues.Storage.Cache.EmptyDir {
		// Add labels to cache PVCs for easier identification during cleanup
		cacheLabels := map[string]string{
			"app.kubernetes.io/name":       req.App.Name,
			"app.kubernetes.io/part-of":    req.App.Namespace,
			"epinio.io/type":               "cache",
			"app.kubernetes.io/managed-by": "epinio",
		}
		err = ensurePVC(ctx, cluster, params.HelmValues.Storage.Cache, req.App.MakeCachePVCName(), cacheLabels)
		if err != nil {
	...
	if !params.HelmValues.Storage.SourceBlobs.EmptyDir {
		// Source blobs PVCs don't need labels for cleanup (they're deleted with app)
		err = ensurePVC(ctx, cluster, params.HelmValues.Storage.SourceBlobs, req.App.MakeSourceBlobsPVCName(), nil)
		if err != nil {
