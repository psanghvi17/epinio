...
const (
	pvcWaitInterval = time.Second
	pvcWaitTimeout  = 30 * time.Second
)
...
// ensurePVC creates or recreates the PVC used for staging sources/cache.
// If the PVC exists but is invalid (wrong size/class/mode or not bound) it
// will be deleted and recreated with the desired settings.
func ensurePVC(ctx context.Context, cluster *kubernetes.Cluster, config StagingStorageValues, pvcName string) error {
	applyPVCDefaults(&config)

	pvc, err := cluster.Kubectl.CoreV1().PersistentVolumeClaims(helmchart.Namespace()).
	...
	if err == nil {
		...
		// Give Pending PVCs a chance to bind before recreating.
		if pvc.Status.Phase == corev1.ClaimPending {
			if err := waitForPVCBound(ctx, cluster, pvcName); err == nil {
				pvc, err = cluster.Kubectl.CoreV1().PersistentVolumeClaims(helmchart.Namespace()).
					Get(ctx, pvcName, metav1.GetOptions{})
				if err != nil && !apierrors.IsNotFound(err) {
					return errors.Wrap(err, "re-fetching pvc after wait")
				}
				if err == nil {
					if valid, _ := pvcMatchesConfig(*pvc, config); valid {
						return nil
					}
				} else {
					reason = "pvc missing after wait"
				}
			} else {
				reason = err.Error()
			}
		...
		inUse, checkErr := pvcInUseByPods(ctx, cluster, pvcName)
		if checkErr != nil {
			return checkErr
		}
		if inUse {
			return apierror.NewBadRequestError(fmt.Sprintf("pvc %s is in use by active pods; retry later", pvcName))
		}

		log := requestctx.Logger(ctx)
		log.Info("recreating invalid staging pvc (cache will be rebuilt)", "pvc", pvcName, "reason", reason)

		if err := deletePVCAndWait(ctx, cluster, pvcName); err != nil {
			return err
		}
	}
...
// applyPVCDefaults ensures required fields are set before creation/validation.
func applyPVCDefaults(config *StagingStorageValues) {
	if config == nil {
		return
	}
	if config.Size == "" {
...
// pvcMatchesConfig checks that an existing PVC matches the desired staging storage settings.
func pvcMatchesConfig(pvc corev1.PersistentVolumeClaim, config StagingStorageValues) (bool, string) {
	...
	desiredSize, err := resource.ParseQuantity(config.Size)
	if err != nil {
		return false, fmt.Sprintf("invalid desired size: %s", err)
	}

	currentSize, ok := pvc.Spec.Resources.Requests[corev1.ResourceStorage]
	if !ok {
		return false, "pvc missing storage request"
	}
	...
	if len(config.AccessModes) > 0 {
		if !accessModesEqual(pvc.Spec.AccessModes, config.AccessModes) {
			return false, "access modes mismatch"
		}
	} else if len(pvc.Spec.AccessModes) > 0 {
		return false, "unexpected access modes on pvc"
	}

	if config.VolumeMode != "" {
		if pvc.Spec.VolumeMode == nil || *pvc.Spec.VolumeMode != config.VolumeMode {
			return false, "volume mode mismatch"
		}
	}

	if config.StorageClassName != "" {
		if pvc.Spec.StorageClassName == nil || *pvc.Spec.StorageClassName != config.StorageClassName {
			return false, "storage class mismatch"
		}
	}
...
func accessModesEqual(current, desired []corev1.PersistentVolumeAccessMode) bool {
	if len(current) != len(desired) {
		return false
	}

	found := make(map[corev1.PersistentVolumeAccessMode]bool, len(current))
...
func waitForPVCBound(ctx context.Context, cluster *kubernetes.Cluster, pvcName string) error {
	waitCtx, cancel := context.WithTimeout(ctx, pvcWaitTimeout)
	defer cancel()

	err := wait.PollImmediateWithContext(waitCtx, pvcWaitInterval, pvcWaitTimeout, func(ctx context.Context) (bool, error) {
		pvc, err := cluster.Kubectl.CoreV1().PersistentVolumeClaims(helmchart.Namespace()).
			Get(ctx, pvcName, metav1.GetOptions{})
		...
		case corev1.ClaimLost:
			return false, errors.Errorf("pvc %s in lost phase", pvcName)
		default:
			return false, nil
		}
	})
	if err != nil {
		return errors.Wrapf(err, "waiting for pvc %s to bind", pvcName)
	}
	return nil
}

func deletePVCAndWait(ctx context.Context, cluster *kubernetes.Cluster, pvcName string) error {
	policy := metav1.DeletePropagationForeground
	delErr := cluster.Kubectl.CoreV1().PersistentVolumeClaims(helmchart.Namespace()).
		Delete(ctx, pvcName, metav1.DeleteOptions{PropagationPolicy: &policy})
	...
	if err != nil {
		return errors.Wrapf(err, "waiting for pvc %s deletion", pvcName)
	}
	return nil
}

func pvcInUseByPods(ctx context.Context, cluster *kubernetes.Cluster, pvcName string) (bool, error) {
	pods, err := cluster.Kubectl.CoreV1().Pods(helmchart.Namespace()).
...
func getBlobUID(ctx context.Context, s3ConnectionDetails s3manager.ConnectionDetails, req models.StageRequest, app *unstructured.Unstructured) (string, apierror.APIErrors) {
	...
	if req.BlobUID != "" {
		...
		if err != nil {
			returnErr = apierror.InternalError(err, "looking up the previous blob UID")
			return "", returnErr
		}
	}
